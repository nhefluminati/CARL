...
class EnsemblePredictor:
    def __init__(self, models: list[torch.nn.Module], device: str | torch.device = "cpu"):
        self.models = models
    ...
    def predict_mean_and_members(self, loader):
        ...
        for x, y, w in loader:
            ...
            for i, model in enumerate(self.models):
                member_scores[i].append(model(x_dev).flatten().detach().cpu().numpy())
            labels.append(y.numpy().flatten())
            weights.append(w.numpy().flatten())
            features.append(x.numpy())

        member_scores = np.stack([np.concatenate(s) for s in member_scores], axis=0)
        return {
            "scores_members": member_scores,
            "scores_mean": member_scores.mean(axis=0),
            "scores_std": member_scores.std(axis=0, ddof=1) if len(self.models) > 1 else np.zeros(member_scores.shape[1]),
...
class CARLDiagnostics:
    ...
    def plot_training_diagnostics(self, histories: list[dict]) -> None:
        ...
        for hist in histories:
            member = hist["member"]
            ax1.plot(np.arange(len(hist["train_loss"])), hist["train_loss"], alpha=0.35, label=f"train m{member}")
            ax1.plot(np.arange(len(hist["val_loss"])), hist["val_loss"], alpha=0.35, linestyle="--", label=f"val m{member}")
            ax2.plot(np.arange(len(hist["val_norm"])), hist["val_norm"], alpha=0.45, label=f"norm m{member}")
        ax1.set_ylabel("Loss", fontsize=12)
        ax1.set_title("Training / Validation Objective per Ensemble Member")
        ax1.grid(alpha=0.3)
    ...
    def plot_roc(self, scores: np.ndarray, labels: np.ndarray, weights: np.ndarray | None = None) -> float:
        # Match the original single-network diagnostic: sklearn ROC/AUC is
        # evaluated unweighted. HEP MC weights can be signed or have tiny
        # numerical pathologies; passing them to roc_curve can make the FPR
        # non-monotonic, which causes sklearn.metrics.auc to fail.
        ...
        plt.title("ROC Curve: Ensemble Mean")
        plt.legend()
        plt.tight_layout()
    ...
    def plot_calibration(self, p: np.ndarray, t: np.ndarray, w: np.ndarray, s_nbins: int = 30) -> None:
        ...
        for i in range(s_nbins):
            mask = (p >= s_bins[i]) & (p < s_bins[i + 1]) if i < s_nbins - 1 else (p >= s_bins[i]) & (p <= s_bins[i + 1])
            t_bin = t[mask]
            w_bin = w[mask]
            sig_mask = t_bin == 1.0
            bkg_mask = t_bin == 0.0
        ...
        plt.errorbar(s_bin_centers[valid], s_true[valid], yerr=s_err[valid], fmt="o", capsize=3, label="True weighted signal fraction")
        plt.plot([0, 1], [0, 1], "--", label="Ideal: truth = bin center")
        plt.xlabel("ensemble mean NN output bin center")
    ...
    def plot_reweighting_closure(
        ...
        h_num_carl, _ = np.histogram(xobs_denominator, bins=xbins, weights=w_denominator * r_denominator)
        h_num_mc_var, _ = np.histogram(xobs_numerator, bins=xbins, weights=w_numerator ** 2)
        h_num_carl_var, _ = np.histogram(xobs_denominator, bins=xbins, weights=(w_denominator * r_denominator) ** 2)

        h_num_mc_density = h_num_mc / xwidths
        h_denom_density = h_denom / xwidths
        h_num_carl_density = h_num_carl / xwidths
        ratio_num_mc_over_denom = np.divide(h_num_mc, h_denom, out=np.full_like(h_num_mc, np.nan, dtype=float), where=h_denom > 0)
        ratio_num_carl_over_denom = np.divide(h_num_carl, h_denom, out=np.full_like(h_num_carl, np.nan, dtype=float), where=h_denom > 0)
        ratio_carl_over_mc = np.divide(h_num_carl, h_num_mc, out=np.full_like(h_num_carl, np.nan, dtype=float), where=h_num_mc > 0)
        ratio_carl_over_mc_err = np.divide(np.sqrt(h_num_carl_var), h_num_mc, out=np.full_like(h_num_carl, np.nan, dtype=float), where=h_num_mc > 0)

        fig, (ax1, ax2, ax3) = plt.subplots(3, 1, gridspec_kw={"height_ratios": [2, 1, 1]}, figsize=(5, 6), sharex=True)
        ax1.stairs(h_num_carl_density, xbins, color="cornflowerblue", linewidth=1.5, label="Target from CARL-reweighted reference")
        ax1.stairs(h_num_mc_density, xbins, color="blue", linestyle="--", linewidth=1.5, label="Target MC")
        ax1.stairs(h_denom_density, xbins, color="black", linestyle="--", linewidth=1.5, label="Reference MC")
        ...
        valid = h_num_mc > 0
        ax3.errorbar(xcenters[valid], ratio_carl_over_mc[valid], yerr=ratio_carl_over_mc_err[valid], xerr=xwidths[valid] / 2.0, fmt="none", color="cornflowerblue", linewidth=1.5)
        ...
        plt.savefig(self.output_dir / f"carl_reweight_ensemble_{feature_name}_{self.run_name}.pdf", bbox_inches="tight")
        plt.close()

    def write_member_spread(self, scores_members: np.ndarray) -> None:
        plt.figure(figsize=(7, 5))
        spread = scores_members.std(axis=0, ddof=1) if scores_members.shape[0] > 1 else np.zeros(scores_members.shape[1])
        plt.hist(spread, bins=50)
        plt.xlabel("per-event ensemble std of NN output")
        plt.ylabel("Events")
